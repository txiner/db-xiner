...
def select(*project, **kwargs):
    ...
    * a :class:`Table <hustle.Table>` instance when :code:`nest==True`
    ...
    if nest:
        ...
        _print_separator(80)
        _printline(f,cols, width=80, cols=len(cols),
...
def dump(result_urls,f, width=80):
    """
    ...
    for columns, _ in result_iterator(result_urls):
        ...
        _printline(f,columns, width=width, cols=len(alignments), alignments=alignments)
...
def _printline(f,items, width=80, cols=3, alignments=None):
    i = 0
    line = ""
    if alignments is None:
        alignments = [_ALG_CENTER] * max(cols, len(items))
